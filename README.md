...
| 文件 | 放什么 | 模板 | 谁加载 |
| --- | --- | --- | --- |
| `~/.uzqw.dotfiles.env` | 密钥、每机不同的地址（如 `AW_APIURL_HOST`） | `.env.example` | `zsh/.zshenv` |
...
两个文件都在 `$HOME`，仓库管不到，`git clean -fdx` 或重新 clone 都不会碰它们，内容
请自行备份。仓库工作树里不放任何机器专属文件，所以 `git clean -fdx` 是安全的。
