...
-- Native number column (no custom statuscolumn). Avoids fighting render-markdown.
opt.statuscolumn = ''
...
-- Do NOT save localoptions: old sessions stored `norelativenumber` and undid hybrid numbers.
opt.sessionoptions:remove({ 'localoptions', 'options' })
...
-- Markdown is for writing; don't mark words with spell-check underlines.
vim.api.nvim_create_autocmd({ 'FileType', 'BufEnter' }, {
	group = vim.api.nvim_create_augroup('rafi_markdown', { clear = true }),
	pattern = { 'markdown', '*.md' },
	callback = function()
