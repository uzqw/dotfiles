...
-- Yank a project-relative file and line reference for AI agents.
...
end, { silent = true, desc = 'Copy AI reference' })

map('x', '<Leader>ac', function()
	-- Read both live visual endpoints before leaving Visual mode. Unlike the
	-- '< and '> marks, these cannot refer to an older selection.
	local cursor_line = vim.api.nvim_win_get_cursor(0)[1]
	local visual_line = vim.fn.getpos('v')[2]
	require('rafi.util.ai_reference').copy({
		filename = vim.api.nvim_buf_get_name(0),
		first_line = visual_line,
